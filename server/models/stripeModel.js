  ...
  try {
    ...
    //活動資料送資料庫
      ...
      try {
        ...
        // 更新資料庫中的票券庫存
        await query('UPDATE events SET ticketType = ? WHERE eventId = ?', [JSON.stringify(updatedTicketTypes), eventId])
...
const checkTicketAvailability = async (event) => {
  try {
    console.log()
    const [dbEvent] = await query('SELECT ticketType FROM events WHERE eventId = ?', [event.eventId])
